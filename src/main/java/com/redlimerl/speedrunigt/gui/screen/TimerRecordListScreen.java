    ...
    private class TimerSplitListWidget extends ElementListWidget<TimerSplitListWidget.TimerSplitEntry> {
        ...
        private class TimerSplitEntry extends ElementListWidget.Entry<TimerSplitEntry> {
            ...
            public void render(int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
                ...
                drawString(TimerRecordListScreen.this.textRenderer, titleText.asFormattedString(), x, y + 4, 16777215);
                if (timelineTextList.size() > 0) {
                    int count = (int) ((System.currentTimeMillis() - entryCreatedTime) / 2500) % timelineTextList.size();
                    drawString(TimerRecordListScreen.this.textRenderer, resultTimeText.deepCopy().append("  ").append(timelineTextList.get(count)).asFormattedString(), x, y + 13, 16777215);
                } else {
                    drawString(TimerRecordListScreen.this.textRenderer, resultTimeText.asFormattedString(), x, y + 13, 16777215);
                }
