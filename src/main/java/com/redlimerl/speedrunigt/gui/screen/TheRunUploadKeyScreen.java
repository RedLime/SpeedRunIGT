...
public class TheRunUploadKeyScreen extends Screen {
    ...
    public TheRunUploadKeyScreen(Screen parent) {
        super(Text.translatable("speedrunigt.option.therun_gg.edit_upload_key"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidgetHelper.create(width / 2 + 2, height / 2 + 24, 98, 20, ScreenTexts.CANCEL, (button) -> close()));

        addDrawableChild(ButtonWidgetHelper.create(width / 2 - 100, height / 2 + 2, 200, 20, Text.translatable("speedrunigt.therun_gg.get_upload_key"), (button) -> Util.getOperatingSystem().open("https://therun.gg/upload-key")));


        this.uploadKeyBox = new TextFieldWidget(this.textRenderer, this.width / 2 - 110, height / 2 - 32, 220, 20, Text.translatable("speedrunigt.therun_gg.insert_upload_key"));
