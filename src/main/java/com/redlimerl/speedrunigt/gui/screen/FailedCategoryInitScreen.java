...
public class FailedCategoryInitScreen extends Screen {
    ...
    public void render(int mouseX, int mouseY, float delta) {
        ...
        drawCenteredString(this.font, new TranslatableText("speedrunigt.message.failed_add_category", this.fileName).asFormattedString(), width / 2, height / 2 - 35, TEXT_RED);
        drawCenteredString(this.font, new TranslatableText("speedrunigt.message.failed_add_category."+exception.getReason().name().toLowerCase(Locale.ROOT)).asFormattedString(), width / 2, height / 2 - 10, TEXT_WHITE);
        drawCenteredString(this.font, exception.getDetails(), width / 2, height / 2 + 2, TEXT_WHITE);
    }
}
