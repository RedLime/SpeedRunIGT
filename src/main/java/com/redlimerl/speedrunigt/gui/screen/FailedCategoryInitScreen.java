...
public class FailedCategoryInitScreen extends Screen {
    ...
    public void render(DrawContext drawContext, int mouseX, int mouseY, float delta) {
        ...
        drawContext.drawCenteredTextWithShadow(this.textRenderer, Text.translatable("speedrunigt.message.failed_add_category", this.fileName), width / 2, height / 2 - 35, TEXT_RED);
        drawContext.drawCenteredTextWithShadow(this.textRenderer, Text.translatable("speedrunigt.message.failed_add_category."+exception.getReason().name().toLowerCase(Locale.ROOT)), width / 2, height / 2 - 10, TEXT_WHITE);
        drawContext.drawCenteredTextWithShadow(this.textRenderer, exception.getDetails(), width / 2, height / 2 + 2, TEXT_WHITE);
    }
}
