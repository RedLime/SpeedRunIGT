    ...
    protected void init() {
        ...
        for (OptionButtonFactory factory : optionButtonFactoryList) {
            ...
            if (!categorySelectButtons.containsKey(category)) {
                ButtonWidget buttonWidget = new ButtonWidget(width - 110, 30 + ((categoryCount++ % 6) * 22), 80, 20, new TranslatableText(category).asFormattedString(), (ButtonWidget buttonWidget1) -> selectCategory(category));
                categorySelectButtons.put(category, buttonWidget);
                addButton(buttonWidget);
            }
        }

        prevPageButton = addButton(new ButtonWidget(width - 110, 30 + (6 * 22), 38, 20, "<", (ButtonWidget button) -> openPage(-1)));

        nextPageButton = addButton(new ButtonWidget(width - 68, 30 + (6 * 22), 38, 20, ">", (ButtonWidget button) -> openPage(+1)));
