...
public class TimerCustomizeScreen extends Screen {
    ...
    @Override
    protected void init() {
        addDrawableChild(new ButtonWidget(width / 2 - 60, height / 2 - 48, 120, 20, new TranslatableText("speedrunigt.option.timer_position.toggle_timer").append(" : ").append(drawer.isToggle() ? ScreenTexts.ON : ScreenTexts.OFF), (ButtonWidget button) -> {
        ...
        addDrawableChild(new SliderWidget(width / 2 - 60, height / 2 - 26, 120, 20, new TranslatableText("speedrunigt.option.timer_position.scale").append(" : ").append(((int) (drawer.getScale() * 100)) + "%"), (drawer.getScale() - 0.5f) / 2.5f) {
            @Override
        ...
        addDrawableChild(new SliderWidget(width / 2 - 60, height / 2 - 4, 120, 20, new TranslatableText("speedrunigt.option.timer_position.background_opacity").append(" : ").append(((int) (drawer.getBgOpacity() * 100)) + "%"), drawer.getBgOpacity()) {
            @Override
            protected void updateMessage() {
        ...
        addDrawableChild(new ButtonWidget(width / 2 - 60, height / 2 + 18, 120, 20, new TranslatableText("speedrunigt.option.timer_position.top_timer").append(" : ").append(drawer.isReversed() ?  "RTA" : "IGT"), (ButtonWidget button) -> {
            drawer.setStatus(drawer.getXPos(), drawer.getYPos(), drawer.getScale(), drawer.getBgOpacity(), !drawer.isReversed(), drawer.isSimply(), drawer.isToggle());
            changed = true;
        ...
        addDrawableChild(new ButtonWidget(width / 2 - 60, height / 2 + 40, 120, 20, new TranslatableText("speedrunigt.option.timer_position.show_time_only").append(" : ").append(drawer.isSimply() ? ScreenTexts.ON : ScreenTexts.OFF), (ButtonWidget button) -> {
            drawer.setStatus(drawer.getXPos(), drawer.getYPos(), drawer.getScale(), drawer.getBgOpacity(), drawer.isReversed(), !drawer.isSimply(), drawer.isToggle());
            changed = true;
