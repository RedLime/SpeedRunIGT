...
public class TimerCustomizeScreen extends Screen {
    ...
    protected void init() {
        if (minecraft != null) {
            FontManagerAccessor fontManagerAccessor = (FontManagerAccessor) ((MinecraftClientAccessor) minecraft).getFontManager();
            if (!fontManagerAccessor.getTextRenderers().containsKey(drawer.getTimerFont())) {
                availableFonts.add(drawer.getTimerFont());
            }

            availableFonts.addAll(fontManagerAccessor.getTextRenderers().keySet());
        }
            ...
            if (!fontButton.active && minecraft != null) {
                ...
                for (int i = 0; i < fontSelectButtons.size(); i++) {
                    if (c + i < availableFonts.size()) {
                        ...
                        if (minecraft != null && fontManagerAccessor.getTextRenderers().containsKey(fontIdentifier)) {
                            targetFont = fontManagerAccessor.getTextRenderers().get(fontIdentifier);
                        } else {
